...
### 2025-09-07 12:30 - 13:00
- **日誌排版改善需求**：用戶反映 log 的所有欄位排版不易閱讀
- **問題分析**：
  - 原本使用 `game_data.ListFields()` 只顯示有值的欄位
  - GameFlowData 有 10 個欄位但日誌只顯示部分
  - 硬編碼特定欄位名稱無法跨遊戲通用
- **解決方案演進**：
  1. 嘗試硬編碼所有欄位 → 違反動態適配原則
  2. 使用 `game_data.ListFields()` → 只顯示有值欄位
  3. 最終方案：使用 `game_data.DESCRIPTOR.fields` 動態獲取所有欄位
- **技術實現**：
  ```python
  ...
  ```
- **關鍵特色**：真正的動態適配，無論 GameFlowData 內容如何變化都能自動適配

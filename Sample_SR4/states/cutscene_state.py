"""
GAME_FLOW_CUTSCENE 狀態處理器
處理過場動畫狀態 (EGameFlowState = 19)
"""
...
class CutsceneStateHandler(BaseStateHandler):
    def __init__(self):
        super().__init__("CUTSCENE", GameFlowState.GAME_FLOW_CUTSCENE)
        
        
    def can_handle(self, state: int) -> bool:
        return state == GameFlowState.GAME_FLOW_CUTSCENE
        
    def handle_state(self, state: int, game_data: Any) -> Optional[bytes]:
        if not self.can_handle(state):
            return None
            
        print(f"🎬 Handling CUTSCENE state")

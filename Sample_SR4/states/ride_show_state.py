"""
GAME_FLOW_RIDE_SHOW 狀態處理器
處理騎乘展示狀態 (EGameFlowState = 16)
"""
...
class RideShowStateHandler(BaseStateHandler):
    def __init__(self):
        super().__init__("RIDE_SHOW", GameFlowState.GAME_FLOW_RIDE_SHOW)
        
        
    def can_handle(self, state: int) -> bool:
        return state == GameFlowState.GAME_FLOW_RIDE_SHOW
        
    def handle_state(self, state: int, game_data: Any) -> Optional[bytes]:
        if not self.can_handle(state):
            return None
            
        print(f"🎬 Handling RIDE_SHOW state")
